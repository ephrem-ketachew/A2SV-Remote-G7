...
class Solution:
    def queryResults(self, limit: int, queries: List[List[int]]) -> List[int]:
        ball_color_map = defaultdict(int)
        color_ball_map = defaultdict(set)
        
        ans = []
        for ball, color in queries:
            if ball_color_map[ball] > 0:
                prev_color = ball_color_map[ball]
                color_ball_map[prev_color].remove(ball)
                if not color_ball_map[prev_color]:
                    del color_ball_map[prev_color]
                    
            ball_color_map[ball] = color
            color_ball_map[color].add(ball)
            
            ans.append(len(color_ball_map))
